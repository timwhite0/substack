...
```{r}
read_and_clean_data <- function(year) {
  of_catch_prob <- read_csv(paste0("../data/of_catch_prob_", year, ".csv")) %>%
  ...
  venues <- mlb_schedule(season = year) %>%
              mutate(game_pk = as.factor(game_pk),
                     venue = as.factor(venue_name)) %>%
              select(game_pk, venue)
  of_catch_prob_teams_venues <- of_catch_prob_teams %>%
                                  left_join(venues, by = "game_pk")
  
  return(of_catch_prob_teams_venues)
}
```


```{r}
of_catch_prob_2024 <- read_and_clean_data("2024")
of_catch_prob_2023 <- read_and_clean_data("2023")
```
