...
```{r}
jan26_data <- of_catch_prob_2024 %>%
                mutate(wall_lf_x = dist_lf * cos((90 + 45) * pi / 180),
                       wall_lf_y = dist_lf * sin((90 + 45) * pi / 180),
                       wall_lc_x = dist_lc * cos((90 + 22.5) * pi / 180),
                       wall_lc_y = dist_lc * sin((90 + 22.5) * pi / 180),
                       wall_cf_x = dist_cf * cos((90 + 0) * pi / 180),
                       wall_cf_y = dist_cf * sin((90 + 0) * pi / 180),
                       wall_rc_x = dist_rc * cos((90 - 22.5) * pi / 180),
                       wall_rc_y = dist_rc * sin((90 - 22.5) * pi / 180),
                       wall_rf_x = dist_rf * cos((90 - 45) * pi / 180),
                       wall_rf_y = dist_rf * sin((90 - 45) * pi / 180)) %>%
                mutate(dist_to_lf = sqrt((landing_pos_x - wall_lf_x)^2 + (landing_pos_y - wall_lf_y)^2),
                       dist_to_lc = sqrt((landing_pos_x - wall_lc_x)^2 + (landing_pos_y - wall_lc_y)^2),
                       dist_to_cf = sqrt((landing_pos_x - wall_cf_x)^2 + (landing_pos_y - wall_cf_y)^2),
                       dist_to_rc = sqrt((landing_pos_x - wall_rc_x)^2 + (landing_pos_y - wall_rc_y)^2),
                       dist_to_rf = sqrt((landing_pos_x - wall_rf_x)^2 + (landing_pos_y - wall_rf_y)^2)) %>%
...
jan26 <- ggplot() +
  geom_point(data = jan26_data, aes(x = landing_pos_x, y = landing_pos_y, col = closest_wall)) +
  annotate("text", x = unique(jan26_data$wall_lf_x), y = unique(jan26_data$wall_lf_y),
           fontface = "bold", label = "X") +
  annotate("text", x = unique(jan26_data$wall_lc_x), y = unique(jan26_data$wall_lc_y),
           fontface = "bold",  label = "X") +
  annotate("text", x = unique(jan26_data$wall_cf_x), y = unique(jan26_data$wall_cf_y),
           fontface = "bold",  label = "X") +
  annotate("text", x = unique(jan26_data$wall_rc_x), y = unique(jan26_data$wall_rc_y),
           fontface = "bold",  label = "X") +
  annotate("text", x = unique(jan26_data$wall_rf_x), y = unique(jan26_data$wall_rf_y),
           fontface = "bold",  label = "X") +
  theme_bw() +
  theme(legend.position = "none") +
  theme(axis.title = element_text(face = "bold"),
...
df <- of_catch_prob_2024 %>%
        ...
        filter(stars < 6) %>%
        mutate(route_distance = sqrt((landing_pos_x - start_pos_x)^2 + (landing_pos_y - start_pos_y)^2)) %>%
        filter(abs(route_distance - distance) < 5) %>%
        mutate(close_to_wall = as.factor(abs(ball_distance - relevant_wall_distance) < 5))
model <- glm(out ~ pos*route_direction + wall + route_distance + hang_time + stadium + player,
