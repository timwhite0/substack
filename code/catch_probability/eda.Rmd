...
```{r}
of_catch_prob_2024 %>%
  count(back_bs = as.factor(of_catch_prob_2024$back),
        back_mine = as.factor(ifelse(
          of_catch_prob_2024$route_direction == "back_middle", 
          1, 0)))
```
...
```{r}
of_catch_prob_2024 %>%
  ggplot(aes(x = landing_pos_x - start_pos_x,
             y = landing_pos_y - start_pos_y)) + facet_grid(rows = vars(back), cols = vars(pos)) + geom_point(size = 1, alpha = 0.1)

of_catch_prob_2024 %>% mutate(back2 = ifelse(route_direction == "back_middle", 1, 0)) %>%
  ggplot(aes(x = landing_pos_x - start_pos_x,
             y = landing_pos_y - start_pos_y)) + facet_grid(rows = vars(back2), cols = vars(pos)) + geom_point(size = 1, alpha = 0.1)
```
...
```{r eval = FALSE}
players_to_keep <- of_catch_prob_2024 %>% count(player) %>% filter(n >= 25) %>% pull(player)
...
model <- glm(out ~ pos*route_direction + wall + distance + hang_time + stadium + player,
