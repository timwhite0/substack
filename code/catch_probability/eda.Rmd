...
jan26 <- ggplot() +
  geom_point(data = jan26_data, aes(x = landing_pos_x, y = landing_pos_y, col = as.factor(closest_wall))) +
  annotate("text", x = unique(jan26_data$wall_dist_lf_x), y = unique(jan26_data$wall_dist_lf_y),
           fontface = "bold", label = "X") +
  annotate("text", x = unique(jan26_data$wall_dist_lc_x), y = unique(jan26_data$wall_dist_lc_y),
           fontface = "bold",  label = "X") +
  annotate("text", x = unique(jan26_data$wall_dist_cf_x), y = unique(jan26_data$wall_dist_cf_y),
           fontface = "bold",  label = "X") +
  annotate("text", x = unique(jan26_data$wall_dist_rc_x), y = unique(jan26_data$wall_dist_rc_y),
           fontface = "bold",  label = "X") +
  annotate("text", x = unique(jan26_data$wall_dist_rf_x), y = unique(jan26_data$wall_dist_rf_y),
           fontface = "bold",  label = "X") +
  theme_bw() +
  theme(legend.position = "none") +
  theme(axis.title = element_text(face = "bold"),
...
```{r}
jan26_data %>%
  mutate(wall_ball = ifelse(relevant_wall_distance - ball_distance < 30, 1, 0)) %>%
  ggplot(aes(x = landing_pos_x, y = landing_pos_y, col = as.factor(wall_ball))) + geom_point()
```
