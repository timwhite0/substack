...
arsenals <- teams %>%
              ...
              mutate(pitchmix = "") %>%
              mutate(pitchmix = ifelse(fourseam_mph > 0, paste0(pitchmix, "FF-"), pitchmix)) %>%
              mutate(pitchmix = ifelse(sinker_mph > 0, paste0(pitchmix, "SI-"), pitchmix)) %>%
              mutate(pitchmix = ifelse(cutter_mph > 0, paste0(pitchmix, "FC-"), pitchmix)) %>%
              mutate(pitchmix = ifelse(slider_mph > 0, paste0(pitchmix, "SL-"), pitchmix)) %>%
              mutate(pitchmix = ifelse(changeup_mph > 0, paste0(pitchmix, "CH-"), pitchmix)) %>%
              mutate(pitchmix = ifelse(curve_mph > 0, paste0(pitchmix, "CU-"), pitchmix)) %>%
              mutate(pitchmix = ifelse(splitter_mph > 0, paste0(pitchmix, "FS-"), pitchmix)) %>%
              mutate(pitchmix = ifelse(sweeper_mph > 0, paste0(pitchmix, "ST-"), pitchmix)) %>%
...
jan12 <- arsenals %>%
          ...
          ggplot(aes(x = fct_reorder(pitchmix, desc(n)), y = n)) +
            ...
            labs(x = "Pitch mix", y = "Number of pitchers",
                 title = "Most common pitch mix among MLB pitchers in 2024") +
            theme(axis.title = element_text(face = "bold"),
                  title = element_text(face = "bold"))
